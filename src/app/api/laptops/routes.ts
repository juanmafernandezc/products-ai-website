...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validaciones
    if (!imagen || !categoria || !marca || !nombre || !procesador || !ramgb || !almacenamientogb || !precio || !pulgadas) {
      return NextResponse.json(
    ...
    const query = `
      INSERT INTO Laptops (imagen, categoria, marca, nombre, procesador, ramgb, almacenamientogb, precio, pulgadas, grafica, descripcion)
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const query = `
      UPDATE Laptops 
      SET imagen = $1, categoria = $2, marca = $3, nombre = $4, procesador = $5, 
